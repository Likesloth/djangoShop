...
from ..services.policy import calculate_due_at, can_renew, compute_renew_due_at
...
def quick_checkout(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and borrower and copy is not None and copy.status == BookCopy.STATUS_AVAILABLE and not Loan.objects.filter(copy=copy, returned_at__isnull=True).exists():
